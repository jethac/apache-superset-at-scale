...
def test_simulation_runs_offline_and_reconciles(tmp_path: Path) -> None:
    result = run_simulation(REPO_ROOT / "scope.yaml", tmp_path / "sim.db", event_count=24, seed=7)
...
def test_simulation_exercises_every_outcome(tmp_path: Path) -> None:
    ...
    counts = run_simulation(REPO_ROOT / "scope.yaml", tmp_path / "sim.db", event_count=36)["funnel"]
    assert isinstance(counts, dict)
    for bucket in ("filtered", "deduped", "work_delivered"):
        assert counts[bucket] > 0, f"{bucket} path never exercised"


def test_simulation_is_deterministic(tmp_path: Path) -> None:
    first = run_simulation(REPO_ROOT / "scope.yaml", tmp_path / "a.db", seed=11)
    second = run_simulation(REPO_ROOT / "scope.yaml", tmp_path / "b.db", seed=11)
